...
\]
\textbf{Min-Max Robust Formulation}
\begin{align*}
  \min_{U} &\max_{W} \sum_{i=1}^{N} \ell(x_{i|k}, u_{i|k})\\
  &  \text{s.t.} \quad x_{i+1|k} = A x_{i|k} + B u_{i|k} + w_{i|k}, \\
  &  \forall w_{i|k} \in \mathcal{W}
  \quad x_{i|k} \in \mathcal{X}
  \quad  u_{i|k} \in \mathcal{U}
\end{align*}
...
\ssthl{Disturbance reachable set}
\quad
...
% REMOVE: unneccesary robust  stuff
% TODO: Error dynamics, short and compact
$\Rightarrow$
