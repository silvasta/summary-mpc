...
\begin{definition}[Maximal Control Invariant Set $\mathcal{C}_\infty$]
  ...
  is the maximal control invariant set
...
Often used as a terminal set $\mathcal{X}_f = \mathcal{C}_\infty$ to
guarantee recursive feasibility in MPC formulations.

For nonlinear systems, computing $\mathcal{C}_\infty$ is NP-hard;
approximations (e.g., ellipsoidal sets) are common. In robust MPC,
extend to robust positively invariant (RPI) sets for disturbance handling.
...
\begin{definition}[Robust Positive Invariant Set $\mathcal{O}^\mathcal{W}$]
  ...
  \[
    ...
    f(x,w) \in \mathcal{O}^\mathcal{W},
    \quad \forall w \in \mathcal{W}
  \]
\end{definition}
Given set $\Omega$ and dynamic system $x(k+1) = f(x(k), w(k))$,
\begin{align*}
  \mathrm{pre}^{\mathcal{W}}(\Omega) := & \{x \mid f(x,w)\} \in
...
\subsection{Probabilistic Reachable Sets (PRS)}

For a target probability $p$ ($Pr(x \in \mathcal{R}_k) \ge p$), a PRS
can be constructed using the incremental Lyapunov function and
Markov's inequality:
\[ \mathcal{R}_k = \left\{ x \mid V_\delta(x, z(k)) \le
\frac{1-\rho^k}{1-\rho} \frac{\bar{w}}{1-p} \right\} \]
