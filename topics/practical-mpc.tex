...
\[
  x_s \oplus \mathcal{X}_f \subseteq \mathcal{X}, K\Delta x + u_s \in
  \mathcal{U}, \quad \forall \Delta x \in \mathcal{X}_f
\]
...
\input{diagram/tracking-diagram.tex}
...
\begin{lemma}
  ...
  \begin{align*}
    ...
    \ (\text{for }n_y = n_d)
  \end{align*}
  $\Rightarrow$ Observer output $C\hat{x}_\infty + C_d
  \hat{d}_\infty$ tracks $y_\infty$ without offset
\end{lemma}
  ...
  \begin{sstOnlyFrame}[BrickRed]
    \begin{theorem}
      ...
      \begin{align*}
        ...
        \\
        \hat{x}(k+1)  = & (A + L_x C)\hat{x}(k) + (B_d + L_x
        C_d)\hat{d}(k)              \\
        & + B\kappa(\cdot) - L_x y(k)                                    \\
        \hat{d}(k+1)  = & L_d C \hat{x}(k) + (\mathbb{I} + L_d
        C_d)\hat{d}(k) - L_d y(k)
      \end{align*}
...
\begin{sstTitleBox}{
  ...
  \begin{sstOnlyFrame}
    \begin{equation}
      \min_{uEPS}\sum_{i=0}^{N-1}
      % TODO: reformulate more general with I(...) L2.37
...
\begin{theorem}[Exact Penalty Funtcion]
  $l_\epsilon(\epsilon) = v \cdot \epsilon$ satisfies requirement for
  any $v > \lambda^\star \geq 0$,
  where $\lambda^\star$ is optimal Lagrange multiplier for original problem
\end{theorem}

% HACK: Soft constraint  with ISS
